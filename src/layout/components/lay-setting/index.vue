...
const themeModer = computed(() => $storage?.layout?.themeMode);
...
const showThemeColors = computed(() => {
  return themeColor => {
    return !(
      themeColor === "light" &&
      (isDark.value || menuStyleValue.value === "popular")
    );
  };
});
...
const menuStyleOptions = computed<Array<OptionsType>>(() => {
  return [
    {
      label: "经典",
      tip: "经典风格",
...
/** 设置菜单风格 */
function onMenuStyleChange({ option }) {
  const { value } = option;
  $storage.layout = {
...
function updateTheme() {
  if (themeModer.value !== "system") return;
  if (mediaQueryList.matches) {
    dataTheme.value = true;
  } else {
    dataTheme.value = false;
  }
  dataThemeChange(themeModer.value);
}
  ...
  <LayPanel>
    <div class="p-5">
      ...
      <Segmented
        resize
        class="select-none"
        :modelValue="themeModer === 'system' ? 2 : darkMode ? 1 : 0"
      ...
      <p :class="['mt-5!', pClass]">菜单风格</p>
      <Segmented
        resize
        class="select-none"
        :modelValue="menuStyleValue === 'classic' ? 0 : 1"
      ...
      <p class="mt-5! font-medium text-sm dark:text-white">全屏水印</p>
      <ul class="setting">
        <li>
        ...
        <li v-if="watermarkConfigs.enable" v-motion-fade>
          <span class="dark:text-white">文本</span>
          <el-input
      ...
      <p class="mt-3! font-medium text-sm dark:text-white">界面显示</p>
      <ul class="setting">
        <li>
