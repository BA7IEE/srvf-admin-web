...
1. 当前只完成 typecheck/eslint/build，未做 `main@075eded` 浏览器/dev 后端冒烟。重点未验:**#98 档案掩码编辑锁**（用一个**不含** `member-profile.read.sensitive` 的角色如 org-admin 编辑队员档案,确认证件号/手机禁用 + 保存后真实值不被掩码覆盖）;#34~#98 范围很大,须补一次真实运行验证。

> ~~自检脚本误扫依赖目录~~ 已于 #96 修复（排除 `.git`/`node_modules`/`.claude` 等 + 跨仓 `../` 不校验存在性,普通+strict 双 0/0）。
...
2. `docs/srvf-admin-vnext-blueprint.md` 是 2026-07-06 快照；主线 #37~#98 已完成大量条目（含后端 v0.38/v0.39 适配），蓝图附录 B 的未消费路径应重新对账。
3. Auth 专线 #51 已改动 `src/api/user.ts`、`src/store/modules/user.ts`、`src/utils/http/index.ts`。后续任何 auth 调整仍需按 CLAUDE.md §4 三要素声明。

## P2

1. 旧 zip 包口径仍散落在历史 handoff/README 记录中；新聊天应以 `main@075eded` 为准。
