...
| `.claude/settings.json` → `permissions.ask`          | 交互护栏                        | **Harness 2.0-FE 起 = 真敏感清单(现 18 条:P1 后 16,§13A.11 补 `--delete` 长写法 2)**:auth 主线 4 项（`src/utils/http/**`、`src/utils/auth.ts`、`src/views/login/**`、`src/router/utils.ts`）、`.husky/**`、`.claude/**`（harness 自保护，§13A.6）、本文件，以及 Bash 面 `git branch -D\|--delete` 四条。改动前先询问人类，无人值守等同拒绝。（`platform-config.json` 与 `package.json` 由 `guard.mjs` 动态守，见 §13A.7 / §13A.10。）                                                                                                                                                                                                                                                                |
| `.claude/hooks/guard.mjs`                            | PreToolUse                      | deny 无法表达的动态规则：`pnpm add/remove/update/clean:cache` 等依赖变更与他源 install（§13.2.1）、`--no-verify` / `HUSKY=0`（§13.3.12）、新增 `@ts-ignore` / `@ts-nocheck` / `@ts-expect-error` / `eslint-disable`（§13.3.5 & §13.3.8 · R1-e；token 在源码里拆串构造，故 guard.mjs 不会自我拦截）、源码中硬编码 `VITE_*` fallback（§13.3.13）、`public/platform-config.json` 改值放行 / 增删顶层字段拒绝 / 存疑询问（§13A.7，取代其静态 ask）、`package.json` 仅 scripts 区变更放行 / 依赖区变更拒绝 / 其余询问（§13A.10，取代其静态 ask）、带旗标依赖命令 / `pkg set\|delete` / `sed`·重定向直写 package.json 三类旁门（§13A.11）。每次决定都附带**指向具体文档章节的原因**，便于模型纠偏。        |
...
- **平台侧分支保护（同日维护者亲手开启；Claude 代开被 Claude Code 分类器拦于仓库设置面——闸门按设计工作）**：main 要求 `verify` 检查通过方可合并（strict:false），`enforce_admins` 连管理员令牌一并约束；服务器侧同步禁 force-push / 禁删分支（与 settings 头注 never-allow 同向双保险）。红 PR 自此由 GitHub 拒收，与「合并前 `gh pr checks` 全绿」纪律（AGENTS §5）双层把关。仓库设置不随库版本化，**本行为其唯一仓内记录**；调整 / 解除走 GitHub Settings → Branches 人工操作。

## 13A.11 首轮实测审查修复记录（2026-07-18）

三路独立审查（守门逻辑 / 测试与 CI / 文档一致性）交叉命中 + 逐条人工复现后修复，方向全部为**收紧**：

1. **guard `applyEdits` 忠实模拟 `replace_all`**——此前只替换首处，跨区编辑（首处在 scripts、后续处在依赖区）会被判「scripts-only」假放行（已复现）；并新增「模拟不可复现（old_string 不在盘上文件）一律退 **ask** 安全地板」，package.json 与 platform-config 两路同修。
2. **checkBash 补三类旁门**：带旗标依赖命令（`pnpm -C dir add` / `--filter` 等，旗标可插在子命令前）、`pkg set|delete`（scripts.\* 平价放行 / 依赖区拒绝 / 其余询问）、`sed -i` 与 shell 重定向直写 package.json（一律拒绝，改走 Edit 工具受闸检查）。
3. **settings `git branch*` allow 收窄**为只读五形态（bare / `--list` / `-v` / `-a` / `--show-current`），`--delete` 长写法补入 ask——消除与 never-allow 头注「删分支全词序变体永不入 allow」的矛盾（审查发现的配置真洞）。
4. **CI 增加三步护栏自测**（harness.test / doctor / readtax，置于装依赖前秒级快挂）——守门者自此也被机器守：削弱 guard 的 PR 无法再拿绿勾。
5. **`harness.test.mjs` 62→82 用例**：上述每条修复与既往盲区（MultiEdit 路径、混合变更、受保护字段删除方向、十字段拒绝面拓宽）全部钉死。

文档层残留（约 25 处旧语义，集中于 baseline / 01 / 03 / 12 / 子目录 CLAUDE.md）由同日文档清扫 PR 另行对齐。
