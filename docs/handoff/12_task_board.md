...
| T-022 | #98 队员档案掩码回写防护       | P0     | CODE_DONE       | `profile/{form.vue,utils/hook.ts}`、`api/srvf-member-profile.ts` | typecheck/eslint/build 绿;掩码 UI 待浏览器验                                        | 适配后端 v0.39.0 §F&A-3;squash `075eded` PR #98             |
| T-021 | 刷新 handoff 至 `main@075eded` | P0     | CODE_DONE       | `docs/handoff/**`、根状态文件                                    | 自检普通+strict 双 0/0                                                              | 本次任务;基线 2cca7a3→075eded,#94~#98                       |
| T-017 | 刷新 handoff 至 `main@2cca7a3` | P0     | ARCHIVED        | `docs/handoff/**`、根状态文件                                    | —                                                                                   | 已被 T-021 取代                                             |
| T-018 | 五任务无提示测试（真人）       | P0     | TODO            | 蓝图 §6 验收                                                     | 待执行                                                                              | 非技术干部;卡壳点进 backlog                                 |
| T-013 | handoff 自检脚本排除规则修复   | P0     | CODE_DONE       | `scripts/check_handoff_docs.py`                                  | 普通+strict 双 0 error/0 warning PASS(2026-07-10)                                   | 排除 .git/node_modules/.claude 等;跨仓 ../ 引用不校验存在性 |
...
| ID    | 阻塞原因                 | 需要谁处理         | 解除条件               |
| ----- | ------------------------ | ------------------ | ---------------------- |
| T-013 | ~~自检脚本误扫依赖目录~~ | 已解除(2026-07-10) | 修复并双模式 PASS      |
