...
| `.claude/settings.json` → `permissions.ask`          | 交互护栏               | §13.1 矩阵 ⚠️「改前评估」文件（`dict` / `tenant` / `schedule` / `permission` 演示模块、`home`/`error`/`remaining` 路由、`mock/**`）改动前先询问人类。（`platform-config.json` 改由 `guard.mjs` 动态守，见 §13A.7，不再走静态 ask。）                                                                                                                                                                                                                                                                     |
