...
| 认证       | 3-call 登录（login → me + rbac/me/permissions）+ 主动预刷新已稳定；**未接** `/auth/v1/logout(-all)`，**无** 40100 被动刷新重试（参考实现已存档 `docs/archive/fork-40100-refresh-reference.md`）                                                                       |
| 平行轨     | v7.1.0-p1 zip 与 main **逐字节一致**（已合流，勿再找增量）；**v7.11.0 fork**（2026-07-17 已弃置）——其 4 个组织人事页面 + scoped-authz 范式 + 考勤表单重构**均已移植合流**；唯一残留增量 40100 被动刷新的参考实现存档于 `docs/archive/fork-40100-refresh-reference.md` |
...
| 来源                                                                                                           | 资产                                                                                                                                                                                                                     | 用法                                                                                              |
| -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| **v7.11.0 fork**（2026-07-17 已弃置，增量已全部合流）                                                          | 4 页面（positions/position-rules/supervision-assignments/role-bindings）、`srvf-{position,supervision,role-binding,authz}.ts`、`srvf-scoped-authz.ts` util + `SrvfAuthzExplainDrawer`、考勤表单重构——**均已移植进 main** | 弃置后以 main 为准；40100 刷新参考存档 `docs/archive/fork-40100-refresh-reference.md`（用后即删） |
...
| SRVF UI 设计稿（`<refs-root>/SRVF-ADMIN-WEB-UI-Reference/`，解压工作版即仅存副本，ZIP 原件 2026-07-17 已弃置） | `srvf-tokens.css` 全套品牌色/字体梯度、dashboard 蓝图（KPI 卡/堆叠柱/日历/待命表）                                                                                                                                       | Phase 3 直接引入 tokens；布局按蓝图重排                                                           |
