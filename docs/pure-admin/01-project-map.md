...
完整版参考仓 `vue-pure-admin/src/views/`（`tenant` / `dict` / `schedule` 等）中的数据与字段都是 Pure Admin 作者为了演示而造的，**绝不能作为后端字段、表结构、接口路径的依据**（见主入口"后端 4 大红线"）。这些演示目录在本仓已物理删除（2026-07 拍板），只余完整版供 UI 范式参考。
...
src/views/
├── welcome/ login/ error/
└── srvf/        # 全部 SRVF 业务页（members-domain / activities-domain / org-hr / base-data / content-domain / notification-domain / recruitment-domain / system / workbench 等）
# 注：starter 自带的 permission / dict / tenant / schedule 演示目录已于 2026-07 物理删除（裁决 1 被取代）；UI 范式改参考完整版 vue-pure-admin/src/views/ + 14-index。
```
...
`pnpm build` 由 PR 的 CI（`.github/workflows/ci.yml`）机械把关（本地仅自诊，详 `02-ai-rules.md` §13.3.8）。
