...
| 项                | 当前状态                                                                                                                         |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| 当前代码基准      | `main@075eded` / `origin/main`，2026-07-10 checkout                                                                              |
| 当前 handoff 版本 | `git-main-075eded-handoff-20260710`                                                                                              |
| 最新包状态        | 未重新打包；以当前 Git checkout 为准                                                                                             |
| 相对旧基线        | `2cca7a3..075eded` 合入 #94~#98（交接清理 + 后端 v0.39.0 档案掩码适配 #98），共 40 文件、+351/-189                               |
| 当前验证状态      | BUILD_PASS（#94~#98 typecheck/eslint/build 绿;#81~#93 另有逐 PR 浏览器冒烟;#98 掩码 UI 待浏览器验,见 17）                        |
| 后端权威基准      | `../srvf-nest-api/docs/handoff/admin-web.md` + live `/api/docs-json`；后端已 **v0.39.0**（326 路由 / 198 权限码 / biz-admin 74） |
...
- **同日交接清理 + 后端 v0.39.0 适配（#94~#98,2026-07-10）**：#94 交接刷新、#95 移除旧权限空态垫片（`perm-empty.vue` 零引用,15 处 import 切 `@/srvf-kit`,T-020）、#96 交接自检脚本排除依赖目录（普通+strict 双 0/0,T-013）、#97 清扫残留旧基线表述、**#98 队员档案证件号/手机掩码回写防护**（适配后端 v0.39.0 §F&A-3:`documentNumber`/`mobile` 默认掩码、明文需 `member-profile.read.sensitive`;无该码者编辑档案时两字段禁用+提交剔除,防掩码覆盖真值)。
...
1. 浏览器/dev 后端冒烟：重点验 #98 档案掩码（不含 `member-profile.read.sensitive` 的角色编辑档案,证件号/手机禁用+保存不覆盖真值）;再抽登录、菜单、全局搜索、字典主从、队员账号、组织人事、RBAC 治理面各一条路径。
2. 若继续做产品开发，先基于当前 `main@075eded` 和 live `/api/docs-json`（后端 v0.39.0）重新选一个小切片，不使用 2026-07-05 的 zip 包任务名。

## 六、给下一位 AI 的最小指令

```text
请基于当前 srvf-admin-web Git checkout `main@075eded` 继续。先读 CLAUDE.md、AGENTS.md、project_state.json、docs/handoff/00/01/23/24/29/30、docs/srvf-admin-vnext-blueprint.md、docs/srvf-admin-ux-upgrade-blueprint.md，以及后端 ../srvf-nest-api/docs/handoff/admin-web.md。注意 2026-07-05 与 1aba0da 两版 handoff 均已过期：#34~#98 已合入——组织人事、RBAC 治理、队员账号闭环、srvf-kit 原语层、UX 产品化系列（IA v3 七组菜单/设置中心/授权向导/使用手册/19 页列表外壳迁移）、后端 v0.39.0 档案掩码适配（#98:documentNumber/mobile 默认掩码,明文闸 member-profile.read.sensitive,编辑面剔除掩码回写）均在主线;后端已 v0.39.0（326 路由/198 权限码/biz-admin 74）。不要恢复 /get-async-routes，不启用 asyncRoutes，不改依赖；涉及 auth 主线必须按 CLAUDE.md §4 声明（src/layout/** 现为 ask 闸,编辑仍需人工逐次确认）。
```
