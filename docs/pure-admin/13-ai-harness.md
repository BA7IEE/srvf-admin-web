...
| `.claude/settings.json` → `permissions.ask`          | 交互护栏                        | **Harness 2.0-FE 起 = 真敏感清单(18 条)**:auth 主线 4 项（`src/utils/http/**`、`src/utils/auth.ts`、`src/views/login/**`、`src/router/utils.ts`）、`package.json`（scripts 区有合法需求，依赖区仍被 guard 拦）、`.husky/**`、`.claude/**`（harness 自保护，§13A.6）、本文件，以及 Bash 面 `git branch -D` 两条。改动前先询问人类，无人值守等同拒绝。（`platform-config.json` 仍由 `guard.mjs` 动态守，见 §13A.7。）                                                                                                                                                                                                                                                                                  |
| `.claude/hooks/guard.mjs`                            | PreToolUse                      | deny 无法表达的动态规则：`pnpm add/remove/update/clean:cache` 等依赖变更与他源 install（§13.2.1）、`--no-verify` / `HUSKY=0`（§13.3.12）、新增 `@ts-ignore` / `@ts-nocheck` / `@ts-expect-error` / `eslint-disable`（§13.3.5 & §13.3.8 · R1-e；token 在源码里拆串构造，故 guard.mjs 不会自我拦截）、源码中硬编码 `VITE_*` fallback（§13.3.13）、`public/platform-config.json` 改值放行 / 增删顶层字段拒绝 / 存疑询问（§13A.7，取代其静态 ask）。每次决定都附带**指向具体文档章节的原因**，便于模型纠偏。                                                                                                                                                                                             |
...
| `docs/pure-admin/14-full-version-reference-index.md` | 参考索引（渐进式披露）          | §13.3.1「新页面前必先扫完整版」+ §12.4 完整版参考策略：把本地完整版 **208 演示页 / 26 个 `Re*` 组件 / 「能力→路径」速查 / 重依赖清单**做成**全量可检索索引**（07 是 curated shortlist，14 是 complete enumeration），让「先查范式、再动手」可机械检索而非靠记忆。`CLAUDE.md` §9、`02-ai-rules.md` §13.3.1 与 §16「新增业务页」行、`07` §12 均指向它。属只读参考、非拦截护栏（改它仍走本文件 §13A.6 ask 网关 + §13.2.2 单独 PR）。                                                                                                                                                                                                                                                                    |
...
- 恒读文档同步收口：根 `AGENTS.md` 成为唯一恒读源（六节，含铁律速查 + 决策锁 + 本清单文档版），`CLAUDE.md` 瘦为 Claude 专属薄入口；v1 双文档冻结于 `docs/archive/harness-v1/`。guard 报错信息引用的「CLAUDE.md §4」按 AGENTS §6 重定向表解析（即 v1 编号，全文在归档）。
- **待维护者 true-up**：`02-ai-rules.md` §13.1 矩阵仍是 1.0 行（layout / style / mock / 演示 views / 三个基础路由 / `store/modules/user.ts` 等，其中演示 views 四目录在 Phase 0-b 已物理删除），`harness-doctor.mjs` 对这些行报 MISS 属预期过渡态；矩阵与 doctor 的同步属 §13A.5 复审要点 1，归维护者。
