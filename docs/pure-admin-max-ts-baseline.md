...
开工先按 §13.4 分型：**全量档**（新业务页 / 契约对接 / 触 ⚠️·❌ 邻域 / 无人值守 goal）走 8 步 checklist；**轻量档**（常规小改，零契约面）红线自查 + 收尾 lint/typecheck；**零码档**（纯 docs / 分析）声明即可。不变量：任何 PR 合并前 lint / typecheck / build 三绿。8 步全文与档位定义以 `02-ai-rules.md` §13.4 为唯一版本，本节不再复述。
