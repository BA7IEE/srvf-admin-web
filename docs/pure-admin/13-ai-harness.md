...
| 文件                                         | 类型          | 机械化了哪条规则                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.claude/settings.json` → `permissions.deny` | 静态护栏      | §13.1 矩阵中除 `mock/**`（该 ❌ 行刻意放进 ask 以保留裁决-4 的 `*.demo.ts` 例外）外的所有 ❌ 文件（auth / token / login / http / user / permission / multiTags / app / settings / epTheme store / 路由核心 / `asyncRoutes` / layout / style / plugins / `main` / `App` / config / `package.json` / lockfile / 工程配置 / `.env*`）的 Edit/Write 一律拒绝；`pnpm-lock.yaml` + `dist` 的 Read 拒绝（降噪，呼应 large-codebases「用 ignore/deny 排除生成物」）。                                            |
