...
- Real login is the **3-call** flow: `POST /api/auth/v1/login` → `GET /api/admin/v1/me` + `GET /api/system/v1/rbac/me/permissions`. Verified live (SUPER_ADMIN · 155 permissions at ship time; re-verify against current `/api/docs-json` before relying on an exact count).
