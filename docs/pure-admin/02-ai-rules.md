...
| **新增业务列表 / 表单页**      | **后端 `../srvf-nest-api/docs/handoff/admin-web.md`（任务→端点图 + 轴模型;先判任务页 vs 资源页）**、`07-max-ts-modules.md` §9 范式表、`src/views/tenant/list/index.vue` + `utils/hook.tsx`、`src/views/dict/index.vue` + `utils/hook.tsx`、`src/components/ReDialog/index.vue`、`src/components/RePureTableBar/src/bar.tsx` |
