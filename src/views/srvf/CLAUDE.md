...
- ⛔ Copy backend contracts / mock / RBAC from the full-version reference repo (AGENTS.md §6 · 14-index 行) — UI patterns only.
- ⛔ Touch the router core (`router/index.ts` / `asyncRoutes.ts` are ❌ deny; `router/utils.ts` is ⚠️ ask) or re-enable `asyncRoutes` / add `getMenuList`.
