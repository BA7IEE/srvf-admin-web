...
- **轻量档** — routine small change (copy / style / local logic, no contract surface) → output Step 1 as a one-line red-line self-check + Step 4 file list (tag §13.1 markers) + Steps 9–10; run `pnpm lint && pnpm typecheck` at wrap-up (the Stop hook backstops typecheck); `pnpm build` moves to just before the PR.
...
Invariant: every PR must be `pnpm lint && pnpm typecheck && pnpm build` green before merge.
...
8. **Build** — `pnpm build`; if the task touches routes / menu / permissions, also run `pnpm dev` and self-check. (轻量档: just before the PR; 零码档: waived. The Stop hook auto-runs typecheck on `src/**` changes regardless.)
