...
做一个新业务页：
  ...
  ⑥ 本仓已有更近的范式（`src/views/srvf/**` 范式 A 三件套如队员页 / 主从字典页 dictionaries / 范式 B 作战室 cockpit / `@/srvf-kit` 原语）就优先用本仓的，
     完整版只补「本仓没有的交互」。
```
...
| 我想做的事                   | 完整版路径                    | 中文     | ⛔禁止                                                                                                      |
| ---------------------------- | ----------------------------- | -------- | ----------------------------------------------------------------------------------------------------------- |
| 页面级权限演示               | `permission/page/index.vue`   | 页面权限 | 本仓无 permission 演示页（已删）；页面显隐用路由 `meta.roles` + 真实 RBAC 码；演示角色名不可作正式（裁决3） |
